...
def match_by_rt(data_subsets, rt_tolerance=20/60, matching_column = 'reference_inchikey_14'):
    data_all = pd.DataFrame()
    for datasubset in data_subsets:
        data_all = data_all.append(datasubset)
    data_all.reset_index(inplace= True, drop = True)
    data_all['rt_match_id']= np.arange(len(data_all))
    if matching_column == 'reference_inchikey_14':
        data_all[matching_column]=data_all['reference_inchikey'].str[0:14]
    
    matched_rt = pd.DataFrame()
    print("finding seed...")
    for inchi14 in tqdm(data_all[matching_column].unique()):
        data_temp = string_search(data_all, matching_column,inchi14)
        data_temp['seed_found']=_find_seed_rt(data_temp, rt_tolerance=rt_tolerance)
...
def _find_seed_rt(data_temp, rt_hits_column = 'hits_rt', rt_tolerance = 5/60, iterative = False):
    ...
    if len(data_temp)<=1:
        return(data_temp.iloc[0]['rt_match_id'])
    ...
    if rt_hit_max ==0 and len(data_temp)>1:
        ...
        return(data_temp['ms1_precursor_intensity'].idxmax())
    seed_candidate = num_search(data_temp,rt_hits_column, rt_hit_max, direction='==')
    # return(seed_candidate)
    return(seed_candidate.loc[seed_candidate['ms1_precursor_intensity'].idxmax()]['rt_match_id'])




def _find_hit_rt(data_temp, retention_time = 'retention_time', rt_tolerance=10/60):
