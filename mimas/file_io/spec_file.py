...
def read_one_spectrum(
        ...
        file_type: object = None, ms2_only=False, **kwargs: object) -> dict:
    """
            ...
            if key_candidate in spec_info:
                if key_target != key_candidate:
                    if value_type is None:
                        spec_result[key_target] = spec_info.pop(key_candidate)
                    else:
                        try:
                            spec_result[key_target] = value_type(spec_info.pop(key_candidate))
                        except:
                            continue
                else:
                    if value_type is not None:
                        try:
                            spec_result[key_target] = value_type(spec_info[key_target])
                        except:
                            continue
                break
        else:
